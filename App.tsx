  ...
  useEffect(() => {
    ...
    const init = async () => {
        ...
        try {
            ...
            // Modified: Order ASC so oldest (Week 1) is first (Right in RTL)
            ...
            if (loadedReports.length > 0) {
                ...
                // Modified: Open the last report (newest) by default
  ...
  useEffect(() => {
      const fetchVisits = async () => {
          ...
          try {
              ...
              snapshot.forEach(doc => fetchedVisits.push({ id: doc.id, ...doc.data() } as Visit));
              ...
              if (fetchedVisits.length > 0) {
                  setReports(prev => {
                      ...
                      if (newReports[currentReportIndex]) {
                          newReports[currentReportIndex] = { ...newReports[currentReportIndex], visits: fetchedVisits };
                      }
                      return newReports;
                  });
              }
...
const handleCreateNewReport = () => {
    ...
    // Modified: Append to end (Ascending order)
    setReports(prev => [...prev, newReport]); 
    // Modified: Switch to the new last index
    ...
    if (window.confirm("هل أنت متأكد من حذف هذا التقرير نهائياً؟")) {
        ...
        if (reportId) {
            try {
                ...
                // Modified: Go to the last available report (Newest) or 0
...
const handleBulkFactoryLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const DecorationLayer = ({ isPrint = false }: { isPrint?: boolean }) => {
      if (!report.decorations?.length) return null;
      return (
          <div className={`absolute inset-0 z-0 overflow-hidden pointer-events-none ${isPrint ? '' : 'rounded-[2rem]'}`}>
              {report.decorations.map((deco) => (
                  <div
                      key={deco.id}
                      className={`absolute origin-center ${isEditing && !isPrint ? 'cursor-move pointer-events-auto' : ''}`}
                      style={{
                          left: `${deco.x}%`,
                          top: `${deco.y}%`,
                      ...
                      onMouseDown={(e) => !isPrint && handleDecoStart(e, deco.id)}
                      onTouchStart={(e) => !isPrint && handleDecoStart(e, deco.id)}
                  >
                      <img 
                          src={deco.url} 
                          alt="" 
                          className={`w-full h-auto select-none ${activeDecoId === deco.id && !isPrint ? 'ring-2 ring-indigo-500 ring-dashed rounded-lg bg-indigo-500/10' : ''}`}
                          draggable={false}
                          crossOrigin="anonymous"
                      />
                  </div>
    ...
    <div className="min-h-screen pb-4 relative">
      ...
      {isEditing && activeDecoId && (
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[3000] bg-white shadow-2xl rounded-full px-6 py-3 border border-gray-200 flex items-center gap-6 animate-fade-in no-print">
              ...
              <button onClick={() => updateDecoScale(activeDecoId, -0.1)} className="p-2 hover:bg-gray-100 rounded-full text-brand-primary"><ZoomOutIcon size={24}/></button>
              <button onClick={() => deleteDecoration(activeDecoId)} className="p-2 hover:bg-red-50 rounded-full text-red-500"><Trash2 size={24}/></button>
              <button onClick={() => setActiveDecoId(null)} className="p-2 hover:bg-gray-100 rounded-full text-gray-400"><X size={20}/></button>
          </div>
      )}
      {selectedImage && (
        ...
        <div className="flex-grow relative z-20">
            {visitsLoading ? (
                ...
                report.visits.map((visit: Visit) => (
                    <VisitCard key={visit.id} visit={visit} isEditing={isEditing} onUpdate={handleUpdateVisit} onDelete={handleDeleteVisit} onImageClick={(url) => setSelectedImage(url)} onUploadImages={(files) => handleManualVisitImageUpload(files, visit.id)} onUploadLogo={(file) => handleVisitLogoUpload(file, visit.id)} />
                ))
            )}
            {isEditing && !visitsLoading && (
                <button onClick={handleAddVisit} className="w-full py-4 border-2 border-dashed border-gray-300 rounded-lg text-gray-400 hover:border-brand-primary hover:text-brand-primary flex items-center justify-center gap-2 mb-8 no-print"><Plus size={24} /> إضافة زيارة يدوياً</button>
            )}
                    ...
                    {report.logos.partners.map((partner: PartnerLogo, idx) => (
                        <div key={partner.id} className="relative flex items-center justify-center px-3 border-r border-gray-200 last:border-none">
                             <div className={`relative flex items-center justify-center ${isEditing ? 'cursor-pointer' : ''}`} onClick={() => isEditing && partnerRefs.current[idx]?.click()}>
                                <img src={partner.url} className="object-contain h-8 w-auto max-w-[80px]" alt="" />
